<template>
    <div class="auction-container">
            ...
            <v-col v-for="item in auctionItems" :key="item.auctionId" cols="12" sm="6" md="4">
                <v-card>
                    ...
                    <div class="auction-button">
                        <base-button
                            v-if="item.auctionStatusId === 1"
                            buttonName="수정"
                            type="button"
                            @click="goToModifyAuction(item.auctionId)"
                            width="145px"
                            height="30px"
                        ></base-button>
                        <base-button
        ...
        <v-dialog v-model="cancelModal" max-width="500px">
            <v-card>
                <v-card-title class="headline">경매 취소</v-card-title>
                <v-card-text>
                    <span class="di-content">정말로 이 경매를 취소하시겠습니까?</span>
                </v-card-text>
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <div class="di-button">
...
const loadMemberAuctionItem = async (page) => {
    const res = await axiosInstance.get('/members/info/auctions/items', {
        params: { page },
    });
...
const deleteAuctionItem = async (auctionId) => {
...
export default {
    ...
    methods: {
        async loadInfo() {
            try {
                const memberAuctionItems = await loadMemberAuctionItem(this.currentPage);
                this.auctionItems = memberAuctionItems.auctions;
                this.totalPages = memberAuctionItems.totalPages;
            } catch (error) {
                console.error(error);
            }
        },
        ...
        goToModifyAuction(auctionId) {
            this.$router.push(`/auction/${auctionId}/modify`);
        },
        ...
        async confirmCancel() {
            ...
            try {
                await deleteAuctionItem(this.selectedAuctionId);
                await this.loadInfo();
            } catch (error) {
                console.error(error);
            }
